...
import RowFoodDiscountEdit from "./RowFoodDiscountEdit";

class TblFoodDiscountEdit extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataListTbl: [],
              ...
              ? dataListTbl.map((item, index) => {
                  return (
                    <RowFoodDiscountEdit
                      key={index}
                      dataListTbl={item}
                      onChange={(data) => this.onChangeChecked(data)}
                    />
                  );
                })
              : null}
...
export default TblFoodDiscountEdit;
