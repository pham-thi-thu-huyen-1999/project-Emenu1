  ...
  isClickCheckItem = (id) => {
    ...
    listFoods.map((item) => {
      if (id === item.id) {
        if (!item.isCheck) {
          item.isCheck = true;
          this.setState({ listNewFoodIscheck: listNewFoodIscheck.concat(item), isCheck: !isCheck });
        }
        else {
          item.isCheck = false;
          this.setState({ listNewFoodIscheck: listNewFoodIscheck.filter(li => li.id !== id), isCheck: !isCheck });
        }
      }
      return item
    })
          ...
          <aside className="msgbox grp-btns">
            <div className="popup-foods flex-box">
              {listFoods.map((item, index) => (
                <div className={(item.isCheck) ? "item active" : "item"} key={index} onClick={() => this.isClickCheckItem(item.id)}>
            ...
            <div>
              <Button main type='s1' onClick={() => this.onClosePopup()}>CANCEL</Button>
              <Button main type='s2' onClick={() => this.handleClickOk()}>OK</Button>
            </div>
          </aside>
        </div>
      </div>
