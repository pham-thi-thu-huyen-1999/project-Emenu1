...
class RowFoodDiscountEdit extends Component {
  constructor(props) {
    super(props);
    this.state = {
        ...
        <ul className="row-tbl">
            ...
            <div className="item-row">
              {dataListTbl.totals.length > 0
                ? dataListTbl.totals.map((total, index) => (
                    <span
                      key={index}
                      className="item-foods"
                      onClick={() => this.removeItemFood(total.id)}
                    >
                      {total.name} <FontAwesomeIcon icon={faTimesCircle} />
                    </span>
                  ))
                : null}
            </div>
          ...
          >
            <button
              ...
              onClick={() => this.chooseFoodsClick(dataListTbl.id)}
            >
...
export default RowFoodDiscountEdit;
