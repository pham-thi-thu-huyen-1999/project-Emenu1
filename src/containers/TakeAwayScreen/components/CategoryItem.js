...
export default class CategoryItem extends Component {
  ...
  componentDidUpdate() {
    ...
    //isHasPrice : true khi chon vao suat co gia => khong cho thay doi gia cua mon an
    //chay lai choose nhung khong chay vao TH return
    if (this.state.valueNumberOld !== this.state.valueNumber && item.is_open_price === false) {
      this.choose();
      this.setState({ valueNumberOld: this.state.valueNumber, showQuantity: false })
    } else if (this.state.valueNumberOld !== this.state.valueNumber && this.state.valuePrice !== this.state.valuePriceOld && item.is_open_price === true) {
      this.choose();
      this.setState({ valueNumberOld: this.state.valueNumber, valuePriceOld: this.state.valuePrice, showPopupPriceWithQty: false })
    }
  }
  ...
  choose = () => {
    ...
    if (valueNumber === valueNumberOld && item.unit_item.qty_type === 1 && item.is_open_price === true) {
    ...
    if (item.unit_item.qty_type === 1 && item.is_open_price === true) {
      this.props.choose({
